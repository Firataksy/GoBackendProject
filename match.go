...
func match(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("player_" + strUserID1)
	...
	if match.Score1 > match.Score2 {
		...
		_, user1WinError := rc.Set(context.Background(), "player_"+strUserID1, users1, 0).Result()
	...
	if match.Score1 < match.Score2 {
		...
		_, user2WinError := rc.Set(context.Background(), "player_"+strUserID2, users2, 0).Result()
	...
	if match.Score1 == match.Score2 {
		...
		users1 := jsonConvert(w, user1)
		_, user1DrawError := rc.Set(context.Background(), "player_"+strUserID1, users1, 0).Result()
		...
		users2 := jsonConvert(w, user2)
		_, user2DrawError := rc.Set(context.Background(), "player_:"+strUserID2, users2, 0).Result()
